...
function tudoDashIndividual(req, res) {
  var idViagem = req.params.idViagem;

  viagemModel.tudoDashIndividual(idViagem).then((resultado) => {
    if (resultado.length > 0) {
      res.status(200).json(resultado);
    } else {
      res.status(204).json([]);
...
function buscarUltimasTemp(req, res) {
    ...
    viagemModel.buscarUltimasTemp(idSensor, limite_linhas).then(function (resultado) {
        if (resultado.length > 0) {
            res.status(200).json(resultado);
        } else {
